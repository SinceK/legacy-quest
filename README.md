...
オープニングからマップ、4章のボス戦、最終勝利画面まで、丸みのある2DファンタジーRPGの世界観で統一しています。4章合計45問の問題プールから、選択・穴埋め・対応付け・並べ替え形式の問題に挑戦し、XPと4種類のスキルを成長させます。初級18問を用意し、DIVISION、SECTION、ルーチン、FDから段階的に学べます。REDEFINES、OCCURS、LOW-VALUE／HIGH-VALUE、C言語連携、Pro*C／Pro*COBOL、外字、日本語EBCDICなど、移行現場で問題になりやすい論点も扱います。
...
実務形式の問題では、EBCDICのSI/SO区間、LOW-VALUE／HIGH-VALUEの実バイト、COMP-3の16進表現、FDと固定長レコード、REDEFINESのレコード種別、COPY句を含む依存グラフ、COBOLとCの言語間インターフェース、Pro*C／Pro*COBOLの埋込みSQLを実データやコードから判断します。
