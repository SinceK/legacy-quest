# MVPステージ設計書：COBOL→Java モダナイズ学習ゲーム（v0.4）

作成日：2026-08-04
更新日：2026-08-17（45問、初級18問、COBOL基礎・Oracleプリコンパイラ分野の拡充を反映）
対象：MVP（全4章 各1ステージ）
前提：要件定義書 v0.5 に準拠（移行元＝IBM Enterprise COBOL標準構文／移行先＝Spring Batch・Spring Boot）

各ステージは6〜16問の問題プールで構成し、通常プレイでは最大5問をランダム出題する。すべて選択操作。
...
本設計の全4章・全45問は `src/data/chapters.json` に実装済み。初級18問でDIVISION、SECTION、段落・ルーチン、FD、埋込みSQLなどの基礎を補強し、REDEFINES、OCCURS、FILLER、COPY句、サブルーチン、C言語連携、Pro*C／Pro*COBOL、外字、COMP-3内部表現、日本語EBCDIC、SBCS／DBCS、SI/SO、LOW-VALUE／HIGH-VALUEを扱う。うち10問は実際のレコード定義、16進バイト列、依存グラフ、言語間呼び出し定義、埋込みSQLを解析する実務形式である。実装では画面コンポーネントが扱いやすいよう、以下の名称へ正規化している。
...
ステージ開始前には章専用背景を使った突入・遭遇演出を表示し、問題プールから最大5問をランダム出題する。回答ごとにボスHP、正誤エフェクト、コンボ、XP獲得、問題別回答履歴を更新し、セッション終了後は章クリア画面へ遷移する。全章は最初から選択でき、誤答した問題には復習クエストから再挑戦できる。学習レポートではトピック別・難易度別の理解度・回答範囲・累計正答率を表示し、任意のトピック、または初級・中級・上級・実践の問題だけを特訓できる。
...
- 実データ解析問題：`code` に問題固有のCOBOL定義、16進バイト列、依存関係を保持
...
## 次のステップ
- 全4ステージ・合計45問、演出、キャラクター、復習、学習レポートは実装済み。
- 主要ブラウザ・スマートフォンでのE2E／表示確認を行う。
- 用語集・COBOL／Java対応表、実績・バッジを拡充候補とする。
