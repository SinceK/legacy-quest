# MVPステージ設計書：COBOL→Java モダナイズ学習ゲーム（v0.3）

作成日：2026-08-04
更新日：2026-08-16（28問、全章開放、復習・学習レポート、難易度別特訓を反映）
対象：MVP（全4章 各1ステージ）
前提：要件定義書 v0.4 に準拠（移行元＝IBM Enterprise COBOL標準構文／移行先＝Spring Batch・Spring Boot）

各ステージは4〜9問の問題プールで構成し、通常プレイでは最大5問をランダム出題する。すべて選択操作。
...
本設計の全4章・全28問は `src/data/chapters.json` に実装済み。基本問題に加え、REDEFINES、OCCURS、FILLER、COPY句、サブルーチン、外字、COMP-3内部表現、日本語EBCDIC、SBCS／DBCS、SI/SOを扱う。うち5問は実際のレコード定義、16進バイト列、依存グラフを解析する実務形式である。実装では画面コンポーネントが扱いやすいよう、以下の名称へ正規化している。
...
## 次のステップ
- 全4ステージ・合計28問、演出、キャラクター、復習、学習レポートは実装済み。
