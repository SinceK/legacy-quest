...
各ステージは1つのテーマにつき2〜3問の設問で構成する。すべて選択操作。
...
本設計の全4章・全設問は `src/data/chapters.json` に実装済み。実装では画面コンポーネントが扱いやすいよう、以下の名称へ正規化している。
...
ステージ開始前には章専用背景を使った突入・遭遇演出を表示し、回答ごとにボスHP、正誤エフェクト、コンボ、XP獲得を更新する。全設問終了後は章クリア画面へ遷移し、次章を解放する。
